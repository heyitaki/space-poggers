...
def get_random_accessory(list):
    # print(sum(tup[1] for tup in list))
...
def get_combo(special):
    tribe = (constants.SPECIAL_TRIBE_LIST if special else constants.TRIBE_LIST)[0]
    background = get_random_accessory(
        constants.SPECIAL_BACKGROUND_LIST if special else constants.BACKGROUND_LIST
    )
